...
export default function useGroupChat(groupId: string, token: string) {
    ...
    useEffect(() => {
        async function loadHistory() {
            try {
                const res = await controller.getAll(`${endpoints.messages}/groups/${groupId}/messages`);
    ...
    useEffect(() => {
        socket.emit("group:join", { groupId }, (ack: any) => {
            console.log("[ws] join ack", ack);                 // <— should be { ok: true }
            if (!ack?.ok) console.error("[ws] join failed:", ack?.error);
        });

        const onNew = (m: any) => {
            console.log("[ws] message:new", m);                // <— confirm broadcast
    ...
    const sendMessage = (text: string) =>
        new Promise((resolve, reject) => {
            ...
            socket.emit("message:send", { groupId, text, clientId }, (ack: any) => {
                console.log("[ws] send ack", ack);               // <— should be { ok: true, message: ... }
    ...
    const startTyping = () => {
        ...
        // server will auto-clear when you emit stop (we’ll debounce in the component)
    };

    const stopTyping = () => socket.emit("typing:stop", { groupId });
